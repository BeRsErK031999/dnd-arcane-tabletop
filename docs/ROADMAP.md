...
- компактный блок общей библиотеки на стартовом экране с выбором папки, повторным сканированием и остановкой задачи;
- постоянный реестр источников и индекс `asset-library/asset-catalog.sqlite` на SQLite через `sql.js`;
- двухфазная фоновая очередь `поиск -> обработка` с рекурсивным обходом без перехода по symlink и без копирования оригиналов;
- метаданные `width / height / format / MIME / size / mtime`, потоковый SHA-256 и WebP-превью до 360 px через `sharp`;
- content-addressed layout превью `<sha-prefix>/<sha256>.webp`, повторно используемый для одинакового содержимого;
- инкрементальный fast path по `size + mtime + preview`, scan marker для пропущенных файлов и статус `missing` только после успешно завершённого прохода;
- безопасная отмена без ложной отметки unseen-файлов как отсутствующих и продолжение обычным повторным сканированием;
- IPC/preload contracts и живые snapshot-события прогресса для renderer;
- миграция схемы 3 для `last_seen_scan_id`, атомарная запись SQLite-файла и пакетные транзакции индекса;
- интеграционные тесты постоянства каталога, фильтров/тегов, настоящего Sharp-превью, отмены, возобновления и библиотеки из 220 файлов.

Критерии готовности:

- приложение после перезапуска видит подключённые источники и индексированные записи;
- исходные изображения остаются во внешней папке, внутри приложения создаются только SQLite-каталог и превью;
- удалённый исходник становится `missing` после полного повторного сканирования, но не после отменённого;
- неизменённые изображения не хешируются и не обрабатываются Sharp повторно;
- lint, typecheck, unit/integration tests, production build и браузерная проверка стартового экрана проходят.

## Этап 19. Asset Manager

Статус: выполнено.

Цель: превратить постоянный SQLite-каталог из этапа 18 в рабочую библиотеку для поиска, организации и выбора изображений кампании.

Реализовано:

- виртуализированная адаптивная сетка превью, которая держит в DOM только видимые карточки;
- поиск по имени, относительному пути и тегам, включая регистронезависимый Unicode-поиск через SQLite-функцию `unicode_lower`;
- фильтры по подключённой папке, формату, размеру и доступности исходника;
- подробная карточка ассета с превью, SHA-256, метаданными, источником и относительным путём;
- редактирование нормализованных пользовательских тегов в SQLite без изменения исходного файла;
- выбор ассета для открытой кампании с типом `map / token / portrait / handout / other`;
- отдельная политика `always` для дополнительного включения выбранного файла в автономный экспорт;
- понятные состояния фоновой индексации, пустой библиотеки, отсутствующего исходника, ошибки чтения и ошибки превью;
- IPC/preload contracts для постраничного запроса каталога и изменения тегов;
- адаптивная компоновка панели деталей и собственная прокрутка на узких и невысоких окнах;
- unit/integration tests для запросов каталога, service boundary, campaign binding и виртуализации 10 000 карточек.

Критерии готовности:

- библиотека загружается из постоянного SQLite-каталога и не копирует внешние оригиналы;
- поиск и фильтры выполняются на стороне каталога, а устаревший ответ не перезаписывает новый запрос;
- выбор доступного ассета сохраняет только переносимые метаданные кампании без абсолютного пути в пользовательских полях;
- повторный выбор того же индексированного файла обновляет существующий campaign asset без дубля;
- отсутствующий или нечитаемый источник нельзя добавить, пока для него нет managed copy; его теги остаются редактируемыми;
- lint, typecheck, unit/integration tests, production build и Windows installer проходят.
...
- атомарная установка через staging с проверкой размера и SHA-256 до публикации blob;
- восстановление повреждённого blob из доступного индексированного источника;
- `managed` reference и runtime URL без зависимости от внешнего absolute path;
- синхронизация ссылок при сохранении, импорте и удалении кампании;
- отдельная двухшаговая garbage collection: preview-план, подтверждение и повторная проверка ссылок перед каждым удалением;
- перенос прямого импорта изображений в общее управляемое хранилище;
- восстановление кампании и повторное использование ассета при недоступной внешней папке;
- команда очистки в Asset Manager с отчётом о количестве удалённых/пропущенных файлов и освобождённом месте.

Критерии готовности:

- одинаковый контент хранится физически один раз независимо от имени файла и числа кампаний;
- кампания открывает managed asset после удаления или отключения исходной папки;
- изменение исходника после индексации не может незаметно подменить сохранённый SHA-256;
- используемый blob не удаляется, даже если ссылка появилась между preview и выполнением очистки;
- legacy campaign references продолжают сохраняться без нарушения foreign keys;
- lint, typecheck, unit/integration tests, production build и Windows installer проходят.
...
- транзитивный обход фонов и объектов сцен, токенов, портретов, handouts и player projection для определения реально используемых ассетов;
- добавление ассетов с политикой `always`, даже если они не используются открытой кампанией;
- manifest с логическими id, SHA-256, размером, MIME и относительным безопасным путём;
- package version 2 с отдельными логическими asset entries и уникальными blob entries;
- упаковка каждого уникального SHA-256 один раз независимо от числа ссылок;
- одноразовый preview состава и размера пакета до выбора целевого файла;
- повторная проверка campaign revision, размера и checksum между preview и записью;
- импорт с полной предварительной валидацией, дедупликацией непосредственно в managed store и отчётом о новых, повторно использованных, пропущенных и повреждённых blob;
- совместимый импорт пакета версии 1 из задачи 16.2;
- адаптивное окно состава экспорта на стартовом экране.

Критерии готовности:

- неиспользуемые ассеты с политикой `when-used` не попадают в переносимую копию кампании;
- одинаковый blob записывается в payload один раз и после повторного импорта не дублируется в managed store;
- absolute source paths отсутствуют в package version 2;
- небезопасный путь, повреждённый payload или устаревший preview отклоняются до публикации кампании;
- package version 1 продолжает импортироваться и мигрирует ассеты в managed store;
- lint, typecheck, unit/integration tests, production build, browser smoke и Windows installer проходят.
