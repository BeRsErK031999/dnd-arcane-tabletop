...
На этапе 24 viewport разделён по владельцам состояния. `Scene.canvas.viewport` управляет только рабочей областью мастера. `PlayerScreenState.playerViewport` хранит масштаб и pan публичного экрана, а `PlayerSceneCanvasProjection.viewport` является его display-ready копией. `createPlayerSceneCanvasProjection` не читает master viewport. Оба вида нормализуются в диапазонах `zoom 0.5–3` и `pan -800–800`; центрирование меняет только pan. Legacy campaign без нового поля получает player viewport из старой scene projection либо нейтральный `{ zoom: 1, panX: 0, panY: 0 }`.
