...
- `MasterShell` стал компактной панелью инструментов: на desktop она занимает 82 px, а на узких экранах превращается в горизонтальный dock;
- сцена всегда остаётся основной областью редактора, а `Ассеты`, `Бой`, `Заметки` и `Игроки` открываются поверх неё как функциональные панели;
- повторное нажатие активного пункта или `Escape` закрывает вспомогательную панель и возвращает фокус на `Сцены`;
- блок материалов сцены можно скрыть, чтобы отдать canvas всю доступную ширину;
- создание и переключение сцен вынесено в компактную верхнюю полосу;
- полный CRUD кампаний удалён из редактора: библиотека проектов остаётся единственной точкой создания, переименования, импорта и экспорта;
- кнопка `Проекты` сохраняет изменения перед возвратом, а выбранный проект и активная сцена восстанавливаются при повторном запуске;
- сохранены существующие canvas, fog, measurements, initiative, notes, character cards, player screen и asset-инструменты без изменения persistence contract.

Критерии готовности:

- canvas остаётся главным визуальным фокусом на 1366x768;
- layout не создаёт горизонтальный overflow на 768x720 и 360x720;
- функциональные панели доступны с мыши и клавиатуры, закрываются через `Escape` с восстановлением фокуса;
- возврат в библиотеку и повторный запуск сохраняют выбранный проект и активную сцену;
...
- постоянная горизонтальная полоса слоёв в порядке `Карта -> ГМ -> Токены`;
- UI-only активный слой не записывается в campaign JSON и не создаёт лишние autosave/undo-события;
- адаптер сопоставляет `Карта` с `map/grid/object`, `ГМ` с `master`, `Токены` с `token`, оставляя `fog` и measurements независимыми;
- неактивные объекты нельзя выбрать или переместить, но selection активного слоя восстанавливается при возврате;
- grid controls доступны только на слое `Карта`;
- изображения добавляются на выбранный пользовательский слой, master-объекты всегда создаются скрытыми от игроков;
- master-слой полностью исключён из player projection и имеет 50% opacity, пока активен другой слой;
- любой ассет, добавленный на `Токены`, создаёт token object размером с текущую клетку;
- master render order и player projection сохраняют порядок `Карта -> ГМ -> Токены`, не меняя сохранённый технический стек.

Критерии готовности:

- unit tests подтверждают mapping, render order, master visibility/opacity и token cell size;
- browser flow подтверждает placement на ГМ и Токены, блокировку неактивного слоя и сохранение selection;
- полоса слоёв остаётся видимой и не создаёт horizontal overflow на 1440, 768 и 360 px;
- browser console не содержит errors/warnings;
- lint, typecheck, полный test suite и production build проходят.
...
- нативный раскрывающийся блок `Экран игрока` с понятным состоянием `ON` или `OFF`;
- `ON/OFF` использует typed IPC и открывает/закрывает отдельное player window без создания дубликатов;
- `Fullscreen` отражает фактический IPC status и недоступен, пока окно закрыто;
- `Clear` сохраняет blank state, очищает публичную проекцию, но не удаляет сцену и её player viewport;
- `Активная сцена` публикует только выбранную сцену, а повторная публикация заменяется на `Сцена активна`.
...
- player page в режиме сцены содержит только canvas и собственные controls масштаба/центрирования;
- сохранение карты, сетки, измерений, fog, объектов и token state немедленно переиздаёт проекцию уже опубликованной сцены;
- выбор другой сцены в мастерской не меняет опубликованную сцену до явного действия `Активная сцена`;
- существующая безопасная проекция продолжает исключать слой ГМ, master notes и приватный token state;
- browser smoke-проверка подтвердила отображение опубликованной сцены в отдельном player-view.
...
- радиокнопки слоёв `Карта / ГМ / Токены` поддерживают roving focus, стрелки, `Home` и `End`;
- доступны видимый focus state, семантические `aria-pressed` и нативное раскрытие панели;
- добавлены регрессии для сохранения публичной сцены, live-проекции и очистки player screen;
- выполнены lint, typecheck, полный набор unit tests, production build и сборка installer;
- обновлены `USER_GUIDE.md`, README и текущий статус этапов.
